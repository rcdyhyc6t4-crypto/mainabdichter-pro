...
function taskQuantity(task) {
  const value = Number(task.plannedQuantity || 0);
...
function worksiteSummary(worksite) {
  ...
  return {
    ...
    injectionTypes: [...new Set(hzTasks.map(task => String(task.injectionType || "").trim()).filter(Boolean))],
...
export async function createWorksitePdf(worksite) {
  ...
  const areas = [...new Set((worksite.tasks || []).map(task => task.areaName).filter(Boolean))].join(", ");
  ...
  checkMark(doc, 49, y + 4.1, s.injectionTypes.some(value => /drucklos/i.test(value)), "drucklose Injektion");
  checkMark(doc, 91, y + 4.1, s.injectionTypes.some(value => /druck|niederdruck/i.test(value)), "Druckinjektion");
  ...
  const bottleNotice = s.openBottles > 0
    ...
    : "";
  const notes = [worksite.generalNotes, bottleNotice].filter(Boolean).join("\n");

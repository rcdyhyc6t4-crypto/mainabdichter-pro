...
import { loadWorksites, saveWorksite as persistWorksite, getWorksite, deleteWorksite, createWorksiteFromVisit, createWorksiteFromLexwareQuotation, workDurationMinutes, worksiteMaterialTotals, recalculateWorksiteTask, taskUsesHz, taskUsesHs, taskUsesResin, taskIsTechnical } from "./construction-v229.js";
...
import { createWorksitePdf, createVisitPdf, downloadBlob } from "./pdf-v228.js";
...
function renderV28Dashboard() {
  ...
  const lowProducts = products.filter(product => Number(product.stock || 0) <= Number(product.minimumStock || 0));
  if ($("v287InventorySummary")) {
    $("v287InventorySummary").textContent = lowProducts.length
      ? `${products.length} Artikel · ${lowProducts.length} kritisch`

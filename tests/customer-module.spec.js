...
test("Anfrage wird geführt erfasst und zur Vor-Ort-Besichtigung übergeben", async ({ page }) => {
  ...
  await expect(page.locator("#visitStartTime")).toHaveValue("");
...
test("Vor-Ort-Besichtigung schlägt eine Maßnahme vor und verlangt Messwerte", async ({ page }) => {
  ...
  await page.locator('[data-open-step="4"]').click();
...
test("Besichtigungszusammenfassung muss vor dem Angebot bestätigt werden", async ({ page }) => {
  ...
  await page.locator('[data-open-step="2"]').click();
...
test("Offizieller Rabatt zeigt Steuerwerte, warnt und hebt die Freigabe auf", async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    visit.areas=[{
      id:"area-price",name:"Keller",wallMaterial:"HBL",wallThickness:"30",measurements:[],photos:[],
      measures:[{id:"measure-price",type:"Horizontalsperre",length:"12",wall:"30",spacing:".25",note:""}]
    }];
    localStorage.setItem("mainabdichter_v10_visit",JSON.stringify(visit));
  });
  await page.reload();
  await page.locator('[data-bottom-page="offer"]').click();
  await page.locator(".legal-price-options summary").click();
  await expect(page.locator("#legalRegularGross")).not.toHaveText("–");
  await expect(page.locator("#legalVatAmount")).not.toHaveText("–");
  await expect(page.locator("#legalNetAmount")).not.toHaveText("–");
  await page.locator("#offerPositionsApproved").evaluate(input => {
    input.checked = true;
    input.dispatchEvent(new Event("change", { bubbles:true }));
  });
...
test("Grundriss wird dem Kunden zugeordnet und zu Google Drive hochgeladen", async ({ page }) => {
  ...
  await page.locator("#firstName").fill("Max");
  await page.locator("#visitJumpSelect").selectOption("visitStep5");

...
import { createWallMeasurementGrid, measurementPointState, wallSurveyProgress } from "./wall-survey.js?v=32.21.1";
...
const MAINABDICHTER_APP_VERSION = "32.21.1";
...
async function syncPipedriveDashboard() {
  ...
  try {
    ...
    if (box) box.innerHTML=todayItems.length?todayItems.map(item=>`<button class="compact-row activity-row" data-activity-id="${item.id||""}"><span><strong>${esc(item.dueTime||"ganztägig")}</strong><small>${esc(item.type||"Termin")}</small></span><span><strong>${esc(item.subject||"Termin")}</strong><small>${esc(item.personName||item.location||"")}</small></span></button>`).join(''):`<div class="empty-mini">Heute sind keine Pipedrive-Termine vorhanden.${cachedUpcomingPipedriveActivities.length?` ${cachedUpcomingPipedriveActivities.length} kommende Termine wurden geladen.`:` Pipedrive lieferte ${Number(diagnostics.unique||0)} Aktivitäten, davon ${Number(diagnostics.dated||0)} mit Datum; keine davon liegt ab ${esc(todayIso())}.`}</div>`;
    renderUpcomingAppointments();
  } catch(error) {
    if ($("dashboardAppointmentCount")) $("dashboardAppointmentCount").textContent = "!";
    if (box) box.innerHTML=`<div class="empty-mini error-text">${esc(error.message)}</div>`;
    throw error;
  }
}
...
function renderWorksiteEditor() {
  ...
  ws.tasks.forEach(task => {
    ...
    if (task.type === "Flächensperre") {
      ...
      if (task.actualWidth === undefined || task.actualWidth === null || task.actualWidth === "") task.actualWidth = Number(task.plannedWidth || 0);
      if (task.actualHeight === undefined || task.actualHeight === null || task.actualHeight === "") task.actualHeight = Number(task.plannedHeight || 1);
...
function setCentralSyncGate(mode = "loading", title = "", hint = "") {
  ...
  setup?.classList.toggle("hidden", mode !== "setup" && mode !== "error");
  if (setup) setup.textContent = mode === "error" ? "Verbindung prüfen" : "Verbindung einrichten";
...
async function ensureDriveBootstrap() {
  ...
  try {
    ...
    if (response?.exists && response.backup) {
      ...
      // Niemals mehr den kompletten Gerätestand blind durch Drive ersetzen.
      // Vorhandene Datensätze beider Seiten werden verlustfrei vereinigt.
      if (backupHasBusinessData(localPayload)) {
        preserveUnsyncedLocalCopy(localPayload, response.file?.modifiedTime || "");
      }
      const centralPayload = mergeFullBackupPayload(response.backup, localPayload);
    ...
    const reason = error?.status === 401
      ...
      : error?.status === 503
        ? `Google Drive ist im Worker nicht vollständig eingerichtet: ${error.message}`
        : `Technischer Grund: ${error.message || "unbekannter Fehler"}`;
    setCentralSyncGate(
      "error",
      "Zentraler Datenstand nicht erreichbar",
      `${reason} Die Gerätedaten bleiben geschützt.`
    );
    return false;
  } finally {

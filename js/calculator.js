...
export function calculateMeasure(settings, measure) {
  ...
  if (type === "Harzverpressung") {
    quantity = num(measure.length);
    const holesPerMeter = Math.min(20, Math.max(10, num(measure.resinHolesPerMeter)));
    const includedKgPerMeter = Math.min(5, Math.max(3, num(measure.resinIncludedKgPerMeter)));
